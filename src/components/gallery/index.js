        ...
        <SRLWrapper options={options} key={input._key}>
            <Wrapper columns={columns}>
                    {images.map(image => {
                        return (
                            <div key={image._key}>
                                <StyledGatsbyImage key={image.asset._key} image={image.asset.gatsbyImageData} alt={image.alternativeText} />
                                {image.caption && <Paragraph secondary margin='0.5rem 0' color='var(--color-accent-05)'>{image.caption}</Paragraph>}
                            </div>
                        )
                    })}
            </Wrapper>
