        ...
        <SRLWrapper options={options} key={_key}>
            <Wrapper columns={columns}>
                    {featureItems.map(item => {
                        return (
                            <div key={item._key}>
                                <StyledGatsbyImage image={item.image.asset.gatsbyImageData} alt={item.image.alternativeText} />
                                <H5 color='var(--color-accent-01)' margin='1rem 0'>{item.title}</H5>
                                <Paragraph color='var(--color-accent-01)' margin='1rem 0'>{item.description}</Paragraph>
                            </div>
                        )
                    })}
            </Wrapper>
