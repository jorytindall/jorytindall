...
export function ModuleRenderer({ modules }) {
    const module = modules.map(m => {
        switch (m._type) {
            case "richText": return <PortableText key={m._key} blocks={m} />
            ...
            case "hero": return <p key={m._key}>This is a hero.</p>
            case "portfolioList": return <PortfolioListItem key={m._key} input={m} />
            case "results": return <Results key={m._key} input={m} />
            case "features": return <Features key={m._key} input={m} />
